...
const BeanCalculatorScreen = ({ onBack, showToast }) => {
  ...
  const handleSampleValueChange = (index, sampleIdx, field, val) => {
    ...
    const newArr = [...farms[index][field]];
    newArr[sampleIdx] = val;
    updateFarmData(index, field, newArr);
  };
...
const calculateCabbageResults = (farm) => {
  ...
  // 3. 미보상 비율 적용 최종 피해율 (Final Damage Rate)
  const uncompRatio = floatOrZero(farm.uncompRatio); // Get uncomp ratio
  let finalDamageRate = rawDamageRate;

  if (rawDamageRate > 0 && uncompRatio > 0) {
    // 최종 피해율 = 피해율 - (피해율 * 미보상 비율)
    finalDamageRate = rawDamageRate - (rawDamageRate * uncompRatio) / 100;
  }
  finalDamageRate = Math.max(0, finalDamageRate); // Cannot be negative

  // 4. 유효성 검증: 총 조사계와 총 합계가 일치해야 함
  let error = "";
  if (mismatchSamples.length > 0) {
    // --- 수정된 부분: 오류 메시지에 불일치 표본 번호 포함 ---
    error = `표본 합계 불일치: ${mismatchSamples.join(
      ", "
    )}번 표본의 조사계와 합계가 다릅니다.`;
  } else if (grandTotalPlants !== grandSumCategories && grandTotalPlants > 0) {
    // 이 로직은 보통 mismatchSamples에서 잡히지만, 0이 아닌 경우 대비
  ...
  return {
    ...
    damageRatio: floorTo(damageRatioValue * 100, 2).toFixed(2), // %로 표시 (손해정도비율)
    rawDamageRate: floorTo(rawDamageRate * 100, 2).toFixed(2), // %로 표시 (피해율)
    finalDamageRate: floorTo(finalDamageRate * 100, 2).toFixed(2), // %로 표시 (최종피해율)
    error: error,
...
const CabbageSampleInput = ({
  ...
  const handleLocalKeyDown = (e, fieldKey) => {
    // 임시 ID 생성 후 handleKeyDown 호출
    handleKeyDown(e, farmIdx, `sample${sampleIdx}_${fieldKey}`);
  };

  return (
    // *** w-36 너비 및 Compact 사이즈 유지 ***
    <div
    ...
    >
      ...
      <div className="space-y-1">
        {/* 1. 조사계 */}
        <InputField
          size="compact" // Compact size (폰트/크기 증가)
          ...
          inputMode="numeric"
          disabled={!isEnabled}
          highlight={isMismatch} // 불일치 시 하이라이트
          className="text-sm"
        />
        {/* 2. 정상 */}
        <InputField
        ...
        {/* 13. 피해등급 합계 (정상 + 10%형 ~ 100%형) */}
        <InputField
          size="compact"
          id={`${baseId}total`}
          onKeyDown={(e) => handleLocalKeyDown(e, "total")} // 다음 필드로 이동
          label="합계"
          value={sumCategories.toFixed(0)}
          readOnly={true}
          highlight={isMismatch} // 불일치 시 하이라이트 유지
        ...
        <InputField
          ...
          onKeyDown={(e) => handleLocalKeyDown(e, "damageRatio")} // 다음 표본 혹은 다음 농지로 이동
          label="손해정도비율(%)"
...
const CabbageCalculatorScreen = ({ onBack, showToast }) => {
  ...
  const [currentFarmIndex, setCurrentFarmIndex] = useState(0); // 현재 농지 인덱스 추가
  const [surveyDate, setSurveyDate] = useState(
    new Date().toISOString().split("T")[0]
  );

  // *** 수정된 부분: useRef를 컴포넌트 본문에서 정의 (Hooks 규칙 준수) ***
  const isInitialRender = useRef(true);

  useEffect(() => {
    // surveyDate의 첫 렌더링 시점은 건너뛰기
    ...
    if (hasActiveData) {
      if (
        ...
        // Reset all farm data and index, but keep the new surveyDate
        setFarms(
          Array.from({ length: 17 }, () => ({ ...initialCabbageFarmState }))
        );
  ...
  }, [surveyDate]); // surveyDate가 변경될 때마다 실행
  // **********************************************
  ...
  const updateFarmData = (index, field, value) => {
    setFarms((prevFarms) => {
      ...
      // 2. Recalculate Totals and Results
      ...
      currentFarm.rawDamageRate = results.rawDamageRate; // NEW
      ...
      currentFarm.result = {
        ...
        rawDamageRate: floatOrZero(results.rawDamageRate), // NEW
  ...
  const updateCabbageSampleData = (
    ...
    setFarms((prevFarms) => {
      ...
      // Recalculate results based on new sample data
      const results = calculateCabbageResults(currentFarm);
  ...
  const handleKeyDown = (e, farmIdx, fieldName) => {
    if (e.key === "Enter") {
      ...
      // 1. Virtual Field Order Definition (논리적 순서)
      ...
      // --- End of Virtual Field Order Definition ---
      ...
      if (currIdx !== -1 && currIdx < fields.length - 1) {
        ...
        // Determine the next ID based on whether it's a base/final field or a sample field
        if (
          ...
          // This must be a sample field (e.g., 'sample0_surveyTotal')
          nextId = `cabbage_f${farmIdx}_${nextField}`;
        }
        ...
        if (nextElement && !nextElement.disabled) {
          ...
          // Custom scroll logic for sample cards when moving to a new sample block
          if (nextField.startsWith("sample")) {
            ...
            // Check if we are moving from a non-sample field or to a new sample index
            ...
            if (nextSampleIdx !== currentSampleIdx) {
              ...
              if (sampleContainer && nextSampleCard) {
                ...
                // Scroll container to bring the start of the next card into view
          ...
          // Default scroll behavior for vertical positioning
          nextElement.scrollIntoView({ behavior: "smooth", block: "center" });
        }
      } else if (currIdx === fields.length - 1) {
        // Last field of the farm (uncompRatio)
        if (farmIdx < farms.length - 1) {
  ...
  const handleGlobalReset = () => {
    if (window.confirm("모든 17개 농지의 데이터를 삭제하시겠습니까?")) {
      ...
      setCurrentFarmIndex(0); // 초기화 후 1번 농지로 이동
      // **********************************
    }
  };
